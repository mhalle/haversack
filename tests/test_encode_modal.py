"""Encode jobs on Modal (2026-09-23): the api side and the routing, as plain Python with the
jobs Dict and the volumes replaced by doubles, the way tests/test_modal_app.py drives them.
What a deployed EncodeWorker does is the smoke's business (AGENTS.md); what is held here is
that the api keys, records and routes an encode job as the local server does:
...
- the spawn and the prefetcher agree on whose job it is: an encode job of a task-named encoder
...
def test_whose_job_it_is_is_one_answer():
    ...
    assert m._worker_of({"task": "ts.v2:total_fast", "kind": "encode"}) == m.ENCODE_WORKER
    assert m._worker_of({"task": "ts.v2:total_fast"}) == "nnunetv2"
    assert m._worker_of({"task": "radar:pretrain", "kind": "encode"}) == m.ENCODE_WORKER


def test_the_nnunet_worker_does_not_warm_an_encode_job(monkeypatch):
    m, fake = _swap_dict(monkeypatch)
    fake["e"] = {"id": "e", "state": "queued", "created": 1, "task": "ts.v2:total_fast",
                 "kind": "encode", "source": [{"kind": "s3", "id": "b/e"}]}
    ...
    assert m._prefetch_candidate("x", engine=m.ENCODE_WORKER)[-1] == "e"


def _executor(monkeypatch, *, encodes=True):
    m, fake = _swap_dict(monkeypatch)
    ...
    ex = m.ModalExecutor()
    monkeypatch.setattr(ex, "encodes", encodes)
...
def test_an_encode_submit_records_keys_and_routes_as_encode(monkeypatch, tmp_path):
    ...
    ex.submit("j1", tmp_path / "j1", None, "ts.v2:total_fast", {"int8": True},
              identity=("idc:1",), kind="encode", deliverables=["preview"])
    ex.submit("j2", tmp_path / "j2", None, "ts.v2:total_fast", {}, identity=("idc:1",))
    assert fake["j1"]["kind"] == "encode" and fake["j1"]["deliverables"] == []
    assert "kind" not in fake["j2"]
    assert fake["j1"]["cache_key"] == result_key(("idc:1",), "ts.v2:total_fast", {"int8": True},
                                                 ["encode-versions"], kind="encode")
    assert fake["j2"]["cache_key"] == result_key(("idc:1",), "ts.v2:total_fast", {},
                                                 ["segment-versions"])
    assert spawned == [("ts.v2:total_fast", "j1", "encode"), ("ts.v2:total_fast", "j2", "segment")]
    assert ex.status_of("j1")["kind"] == "encode" and "kind" not in ex.status_of("j2")


def test_a_deployment_without_the_worker_refuses_the_kind(monkeypatch, tmp_path):
    m, fake, ex, spawned = _executor(monkeypatch, encodes=False)
    with pytest.raises(ValueError):
        ex.submit("j1", tmp_path / "j1", None, "radar:pretrain", {}, identity=("idc:1",), kind="encode")
...
def test_the_scratch_fallback_reads_the_field_by_its_name(monkeypatch, tmp_path):
    ...
    (tmp_path / "scratch" / "j1" / FIELD_NAME).write_bytes(b"PK")
    fake["j1"] = {"id": "j1", "task": "radar:pretrain", "kind": "encode", "state": "done"}
    state, path = ex.result_file("j1")
    assert state == "done" and path.name == FIELD_NAME and path.read_bytes() == b"PK"


def test_the_spawn_refuses_encode_without_the_worker(monkeypatch):
    from haversack import modal_app as m
    monkeypatch.setattr(m, "ENCODE", False)
    with pytest.raises(RuntimeError, match="HAVERSACK_ENCODE"):
        m._spawn_worker("radar:pretrain", "j", kind="encode")
