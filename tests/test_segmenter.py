...
from haversack.segmenter import POLICY, Segmenter
...
def _stub_model(monkeypatch):
    _FakeModel.built = 0
    monkeypatch.setattr("haversack.network.TorchModel", _FakeModel)
    yield
...
def test_every_policy_key_is_accepted_as_an_override(monkeypatch):
    ...
    monkeypatch.setattr("haversack.pipeline.segment", lambda image, task, **kw: kw)
    seg = Segmenter()
...
def test_a_path_is_coerced_into_a_store(tmp_path):
    from haversack.weights import WeightsStore, as_store
    s = as_store(tmp_path)
