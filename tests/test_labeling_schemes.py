...
def test_every_declared_scheme_is_whole_and_keeps_the_release_out_of_its_uri():
    ...
    for eco, task, s in seen:
        assert set(s) == {"key", "name", "system_uri", "version", "definition_url"}, (eco.name, task)
        assert all(isinstance(v, str) and v for v in s.values()), (eco.name, task, s)
        assert s["version"] != "unknown"
        assert s["version"] not in s["system_uri"], f"{eco.name}:{task} puts the release in its uri"
        assert s["system_uri"].split("#")[0].startswith("https://") and "#" in s["system_uri"]
...
def test_one_uri_is_one_class_list():
    ...
    for eco, task, s in _every_scheme():
        by_uri.setdefault(s["system_uri"], []).append(f"{eco.name}:{task}")
...
def test_unknown_tasks_raise_and_catalogs_without_a_class_list_declare_none():
    ...
    assert declaring == {"ts.v2", "ts.v3", "moose", "mrsegmentator", "dentalsegmentator",
                         "totalvibe", "cads", "fastsurfer", "monai"}
    for eco in ec.known_ecosystems():
...
def test_moose():
    ...
    assert s["system_uri"] == "https://github.com/ENHANCE-PET/MOOSE#clin_ct_organs"
    assert s["definition_url"].startswith("https://github.com/ENHANCE-PET/MOOSE/releases/tag/moosez-v.")
...
def test_mrsegmentator_versions_by_the_weights_not_the_source_tag():
    ...
    assert s["version"] == "1.2" and s["definition_url"].endswith("/tree/v1.2.0")
    assert s["system_uri"] == "https://github.com/hhaentze/MRSegmentator#base"


def test_dentalsegmentator_is_identified_by_the_concept_doi():
    s = ec.DentalSegmentatorEcosystem().labeling_scheme("base")
    assert s["system_uri"] == "https://doi.org/10.5281/zenodo.10829674#base"
    assert s["version"] == "v100" and s["definition_url"] == "https://zenodo.org/records/10829675"
...
def test_totalvibe_uses_the_repositorys_own_casing_and_shares_vibes_scheme():
    ...
    assert a["system_uri"] == "https://github.com/robert-graf/VIBESegmentator#100:vibe"
...
def test_cads_names_are_upstreams_value_for_value():
    ...
    for task in eco.tasks():
        ...
        dsid = s["system_uri"].partition("#")[2].partition(":")[0]
        theirs = {int(v): n for v, n in upstream[dsid].items() if int(v) != 0}
        assert _classes(f"cads:{task}") == theirs, task
        assert s["version"] == "cads-model-open_v1.0.0" and s["definition_url"].endswith(s["version"])
        uris.add(s["system_uri"])
    assert len(uris) == 9
...
def test_fastsurfer_codes_are_the_ids_and_a_bilateral_channel_has_none():
    ...
    assert s["system_uri"] == "https://github.com/Deep-MI/FastSurfer#v2:asegdkt"
...
def test_monai_identifies_a_bundle_by_name_and_versions_it_by_release(tmp_path):
    ...
    assert s["system_uri"] == f"https://github.com/Project-MONAI/model-zoo#{task}"
    assert s["version"] in s["definition_url"]


def test_ts_v3_is_its_own_scheme_beside_v2():
    """v3 reuses v2's task names with one class changed (value 26): a different class list,
    so a different system_uri, keyed by the catalog's major version."""
    v2, v3 = ec.TSEcosystem().labeling_scheme("total"), ec.TSv3Ecosystem().labeling_scheme("total")
    assert v3["key"] == "ts.v3:total" and v3["system_uri"].endswith("#v3:total")
    assert v2["system_uri"] != v3["system_uri"]
    assert ec.TSv3Ecosystem().labeling_scheme("total_fast") == v3
