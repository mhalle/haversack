...
from haversack.preprocess import (normalization_fingerprint, normalize, normalize_for,
                              to_model_grid)
...
def test_the_input_is_stamped_with_the_normalization_it_received():
    ...
    _, grid = _grid()
    assert normalize_for(grid, ORGANS)._haversack_normalization == normalization_fingerprint(ORGANS)
    assert normalize_for(grid, RIBS)._haversack_normalization != normalization_fingerprint(ORGANS)
