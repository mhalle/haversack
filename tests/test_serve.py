...
def test_health_and_tasks(tmp_path):
    ...
    assert h["name"] == "haversack" and h["accepting"] is True
...
def test_version_self_report(tmp_path):
    ...
    v = r.json()
    assert v["name"] == "haversack"
    assert v["contract"]                              # a contract version is reported
    assert v.get("version") and v["version"] != "unknown"   # haversack __version__
    assert v.get("started_at")
    # `packages` reports installed DEPENDENCIES (haversack itself is mounted/source, no dist
    ...
    from haversack.engines import registry as _reg
...
def test_idc_not_found_names_probed_buckets(tmp_path, monkeypatch):
    ...
    from haversack.serve import IDC_BUCKETS, _fetch_idc_series
...
def test_real_segmenter_accepts_cancel_override(tmp_path, monkeypatch):
    ...
    from haversack import Segmenter, pipeline
...
def test_202s_carry_progress_headers(tmp_path):
    ...
    assert h.headers["haversack-stage"] == "predict"
    assert h.headers["haversack-fraction"] == "0.420"
    g = client.get(url, headers={"Prefer": "wait=0"})
    assert g.status_code == 202 and g.headers["haversack-fraction"] == "0.420"
...
def test_tcia_source_flattens_and_blocks_zip_slip(tmp_path, monkeypatch):
    ...
    from haversack.sources import TCIASource
...
def test_openneuro_is_a_data_only_source(tmp_path, monkeypatch):
    ...
    from haversack.sources import openneuro_source, registry
...
def test_preview_renders_and_serves(tmp_path, monkeypatch):
    ...
    from haversack.preview import render_preview
...
def test_pending_marker_set_before_entry_visible(tmp_path, monkeypatch):
    ...
    import haversack.preview
    from haversack import serve as serve_mod
    monkeypatch.setattr(haversack.preview, "load_oriented_pair",
...
def test_cache_only_status_rechecks_cache_before_failed(tmp_path):
    ...
    from haversack.serve import CacheOnlyExecutor, ResultCache
    cache = ResultCache(tmp_path / "c")
    calls = {"n": 0}
...
def test_listing_advertises_only_resolvable_links(tmp_path, monkeypatch):
    ...
    from haversack.serve import result_key, weights_versions_of
    seg, ex, client = _idc_app(tmp_path, monkeypatch)
...
def test_prefer_wait_never_nan():
    ...
    from haversack.serve import _prefer_wait_raw
...
def test_the_header_is_parsed_as_a_directive_list_not_a_substring():
    ...
    from haversack.serve import wants_no_cache
...
def test_the_reference_image_is_the_first_declared_channel(tmp_path, monkeypatch):
    ...
    from haversack.serve import reference_input
    seg, client = _multi(tmp_path, monkeypatch)
    blobs = {r: volume_bytes(i + 1) for i, r in enumerate(ROLES)}
...
def test_both_doors_refuse_the_same_options(tmp_path):
    ...
    from haversack.serve import _validate_request
