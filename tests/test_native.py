...
from haversack.preprocess import nonzero_box
from haversack.tasks import TaskSpec, resolve_model_folder
...
def test_configuration_preference_picks_3d_fullres_not_the_first_alphabetically(tmp_path):
    ...
    would select the cascade, which haversack cannot run."""
...
def test_available_folds_keeps_what_exists_and_errors_on_what_does_not(tmp_path):
    from haversack.network import available_folds
...
def test_errors_are_catchable_as_a_family_and_as_the_builtin_they_replace():
    ...
    assert issubclass(errors.ModelNotFound, (errors.HaversackError, FileNotFoundError))
    assert issubclass(errors.UnsupportedModel, (errors.HaversackError, NotImplementedError))
    assert issubclass(errors.InputError, (errors.HaversackError, ValueError))
