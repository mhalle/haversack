...
def duckn_grid(rec, centering="cell"):
    ...
    """
    return grid_reference(rec["direction_xyz"], rec["spacing_zyx"], rec["origin_xyz"],
                          rec["shape_zyx"], centering=centering)
...
def brick_geometry(direction, eff, origin, brick, nb):
    ...
    """
    del nb                                                          # shape is the array's
    return brick_attrs(direction, eff, origin, brick)
...
def attrs(direction, eff, origin, *, list_axis, centering):
    """An array's geometry, through duckn's models (see haversack.ranked_store)."""
...
README = Path(__file__).parent / "data" / "ranked_store_README.md"


def write_readme(store):
    """Drop the format reference into the store (an open ``RankedStore``), as a key - so a
    zip and a directory carry it the same way.
...
def main(argv=None):
    """The command line of ``tools/ranked_build_store.py``."""
    import argparse
