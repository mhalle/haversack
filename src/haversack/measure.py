...
Measured against phantoms with closed-form truth (``haversack.phantoms``, 1.5 mm):
...
def volume_area(margin, spacing, *, clip=None, slab: int = 16) -> tuple[float, float]:
    ...
    ``margin`` is a signed field sampled at voxel CENTERS - :func:`haversack.ranked.margin`
    for one class, or :func:`haversack.ranked.decode_groups` for a union. Zero is the
