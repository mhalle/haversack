...
class Body:
    ...
    :class:`~haversack.grid.Grid` - to a signed value, negative inside. ``bound_mm`` is a
...
def rotation_zyx(az: float, ay: float, ax: float) -> np.ndarray:
    ...
    :class:`~haversack.grid.Grid` carries no direction cosines - orientation belongs to the
