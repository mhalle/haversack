"""What :func:`haversack.segment` hands back.
...
#: The ``.seg.nrrd`` header keys that carry a label map's MEANING. Written by
#: :meth:`Segmentation.save` and read back by :func:`haversack.labelmap.read_label_map`,
#: which imports them from here: a consumer of a label map gates by structure NAME, and a
#: reader with its own spelling of these would find no names the day either side moved
#: (2026-09-20). The per-segment keys are 3D Slicer's, so a segmentation exported from
#: Slicer reads the same way.
PROVENANCE_KEY = "haversack_provenance"
SEGMENT_KEY = "Segment{index}_{field}"          # field: Name, LabelValue, Layer, ...
...
class Segmentation:
    ...
    *input's* orientation - the thing you save. It is deliberately not called ``image``: in this
    ...
    schema: object                      # haversack.values.LabelSchema
    grid: object                        # haversack.grid.Grid - the grid the labels live on
    spec: object                        # haversack.tasks.TaskSpec - what was asked for
    ...
    def save(self, path) -> Path:
        ...
        plus the full provenance as an ``haversack_provenance`` key - one self-contained
    ...
    def _seg_nrrd_metadata(self) -> dict:
        """Slicer ``.seg.nrrd`` keys for every present structure + haversack provenance."""
        import colorsys
        import json
        arr = self.array
        ...
        for i, value in enumerate(present):
            ...
            md[SEGMENT_KEY.format(index=i, field="Name")] = name
            md[f"Segment{i}_NameAutoGenerated"] = "0"
            md[SEGMENT_KEY.format(index=i, field="LabelValue")] = str(value)
            md[SEGMENT_KEY.format(index=i, field="Layer")] = "0"
