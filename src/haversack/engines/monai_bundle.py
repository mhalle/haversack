...
def weights_installed(bundle: str, version: str) -> list[dict]:
    ...
    :meth:`haversack.ecosystems.MonaiEcosystem.weights_identity`); this mirror exists
...
def resolve_label_names(bundle_dir, present) -> tuple[dict, dict]:
    ...
    **haversack does not yet interpret region heads.** Overlapping regions do not fit
    ...
    return ({int(v): f"label {int(v)}" for v in (present or ())},
            {"labels_unnamed": True,
             ...
             "labels_note":
                 ...
                 + " - haversack does not interpret region heads, so the values are "
    ...
    for role, img in images[1:]:
        if not _same_grid(ref, img):
            raise InputError(
                ...
                "submission; haversack does not register images.")
    return images
...
def segment(image_input, bundle: str, *, root, version: str | None = None,
    ...
    """Run a MONAI bundle and return a :class:`haversack.result.Segmentation` on the
    input's grid.
