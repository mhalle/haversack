...
- An **ecosystem** (:mod:`haversack.ecosystems`) is a *catalog*: the thing a user
  selects, with the ``eco:task@version`` grammar. It owns task names, weight
  installation, and describe.
- An **engine** (:mod:`haversack.engines.registry`) is a *runtime*: a container
  image, a compute entry point, and a weights identity. It owns nothing the user
  names.

**Many ecosystems map to one engine.** ``ts.v2``, ``moose``, ``mrsegmentator`` and ``custom`` are four
