...
``segment()`` on its own builds a :class:`~haversack.network.TorchModel` per call and drops it -
...
class ModelCache:
    ...
    Distinct from :class:`~haversack.weights.WeightsStore`: that one owns the *files* and is the
