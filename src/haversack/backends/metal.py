...
def library():
    ...
    if _LIB is None:
        if not available():
            raise RuntimeError("haversack.backends.metal: needs an MPS device and torch.mps.compile_shader (torch >= 2.7)")
        try:
...
def run(logits: torch.Tensor, out: torch.Tensor, tables, lut, *, mode: str, paint: bool, background: int,
    ...
    if logits.device.type != "mps" or out.device.type != "mps":
        raise ValueError("haversack.backends.metal: logits and out must be on the 'mps' device")
    if logits.dtype == torch.float32:
        lt = "float"
    elif logits.dtype == torch.float16:
        lt = "half"
    else:
        raise TypeError(f"haversack.backends.metal: logits must be float32 or float16; got {logits.dtype} (cast first)")
    if out.dtype == torch.uint8:
        ot = "uchar"
    elif out.dtype == torch.uint16:
        ot = "ushort"
    else:
        raise TypeError(f"haversack.backends.metal: out must be uint8 or uint16; got {out.dtype}")
    if not out.is_contiguous():
        raise ValueError("haversack.backends.metal: out must be contiguous")
