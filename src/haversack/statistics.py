...
field measurement (`haversack.measure`) is reported ALONGSIDE it rather than
...
def compute_statistics(image, labels_path, out_json, *, pair=None,
    ...
    ``ranked_code`` is this run's :class:`~haversack.ranked.RankedCode`, optional. Given
...
def _field_measurements(code) -> dict:
    ...
    the area. See :func:`haversack.measure._censored_fit`.
