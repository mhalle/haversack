...
Pydantic stops here and at :mod:`haversack.serve`. It must not reach the value types
(:class:`~haversack.result.Segmentation`, :class:`~haversack.grid.Grid`) or the kernel
...
class ProcessingParams(Params):
    ...
    # Deliberately a subset of haversack.segmenter.POLICY. `device`, `dtype`,
...
def bind_sources(sources: list, inputs, *, multi_input: bool, task: str = "") -> list:
    ...
    if inputs is None:
        raise RequestError(
            "not_bindable",
            f"{what}does not declare which images it takes, so haversack will not "
...
def validate_options(model, options: dict) -> dict:
    ...
    key). Raises :class:`~haversack.errors.RequestError` with our own vocabulary.
    """
    try:
